...
//     // Store each section in arrays
//     const appliedJobs = response.Applied;
//     // const statusUpdates = response.Status;
...
//     const result = await chatSession.sendMessage(`Process the following list of emails: ${str}.Filter only employment mails not swiggy zommato ,bank bazar,github,make my trip or other not relevant mails then Extract and organize the information into the following structure:

//     Summary: A string summarizing the total number of jobs applied , and whether any meetings were scheduled.
//     Applied: A list of all jobs applied for includes linkedin mail with your application was sent to .., including the company name, position, platform name, aplly link some platform names: like glassdoor linkedin etc.
//     ToApply: A list of recommended jobs to apply for, including the company name, position, platform name, and apply link.
// {CompanyName
// : 
// "BNET Insurance Brokers Pvt Ltd",
// Platform
// : 
// "Glassdoor",
// Position
// : 
// "Front End Developer"}
//     Filter and separate the data into 'Applied' (jobs already applied for) and 'ToApply' (jobs recommended for application).`);
...
//               id: email.id, // If you need to retain the id
...
//       setChatRan(true);
//     }
//   }, [formattedEmails,chatRan]);
//   if (status === "loading" || loading || isChatRunning) {
//     return <CustomAlert variant={"default"} headsUp={"Loading.."} description={"LOading"}/>;
//   }
...
//       <TabsDemo firstComponent={ <CustomTable responseData={responseData} />} secondComponent={ <ToApplyJobsTable responseData={responseData} />}/>
